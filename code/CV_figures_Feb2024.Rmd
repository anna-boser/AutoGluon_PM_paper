...
```{r}
sp <- data.frame(group = as.factor(c(1,1,1,1, 2,2,2,2)), Predictions = 8:1, Observations = c(5,6,7,8,1,2,3,4))
ggplot(sp, aes(x = Predictions, y = Observations)) + 
...
ggsave("figures/Simpson_example.pdf", width = 3, height = 3, units = c("in"))
ggsave("figures/Simpson_example.png", width = 3, height = 3, units = c("in"))
...
make_f4_df <- function(cv_df, station_subs = TRUE){
  ...
  r2_df <- df %>% 
    group_by(Model) %>%
    summarise(value = round(mean(r2), 2), 
              low = round(quantile(r2, probs = .025), 2), 
              high = round(quantile(r2, probs = .975), 2), 
              measure = "R2")
  ...
  rmse_df <- df %>% 
    ...
    summarise(value = round(mean(rmse), 2), 
              low = round(quantile(rmse, probs = .025), 2), 
              high = round(quantile(rmse, probs = .975), 2), 
              measure = "RMSE")
  ...
  nse_df <- df %>% 
    ...
    summarise(value = round(mean(nse), 2), 
              low = round(quantile(nse, probs = .025), 2), 
              high = round(quantile(nse, probs = .975), 2), 
              measure = "NSE")
  ...
  df2 <- data %>% group_by(grouping, Model, measure) %>% 
    summarise(low = round(mean(value) - ((1.96*sd(value))/sqrt(n())), 2), 
              high = round(mean(value) + ((1.96*sd(value))/sqrt(n())), 2), 
...
ggsave("figures/Step2.pdf", width = 7, height = 3, units = c("in"))
ggsave("figures/Step2.png", width = 7, height = 3, units = c("in"))
```
...
ggsave("figures/Step1b.pdf", width = 5, height = 3, units = c("in"))
ggsave("figures/Step1b.png", width = 5, height = 3, units = c("in"))
...
ggsave("figures/Step1a.pdf", width = 3.2, height = 3, units = c("in"))
ggsave("figures/Step1a.png", width = 3.2, height = 3, units = c("in"))
```
...
ggplot(subs) + 
  geom_abline(intercept = 0, slope = 1, color = "grey", size = .5)  + 
  geom_point(aes(x = PM_pred, y = PM, color = as.factor(group)), size = .01, alpha = .5) + 
  ...
  xlim(0,50) +
  ylim(0,50) + 
  geom_smooth(method = lm, se = FALSE, aes(x = PM_pred, y = PM, color = as.factor(group)), size = .5, alpha = .5)
```
...
filter(day_df, Model %in% c("2S", "AutoGluon")) %>% 
  ggplot(aes(x = mean_PM, y = value, color = Model)) + 
    scale_color_discrete(labels=c("Linear", "Nonparametric")) + 
    geom_point() + 
    geom_smooth(se = F) + 
...
### Check how changing the problem to estimating PM - mean(daily PM) makes for better predictions
```{r}
cv_df_demean <- read.csv("Data/output/Autogluon_cv_preds_w_spatial_var.csv")
target_stations <- read.csv("Data/datasets/Small_Ids.csv")[,1]

newdf <- make_f4_df(cv_df_demean, station_subs = TRUE)

m.labs <- c("De-meaned", "Regular")
names(m.labs) <- c("AutoGluon_spatial_var", "AutoGluon")
xlabs <- c("Unisolated", "Temporal", "Spatial")
ylabs <- c(TeX(r'(NSE)', bold=TRUE), TeX(r'($r^2$)', italic=TRUE, bold=TRUE))

ggplot(filter(newdf, measure %in% c("R2", "NSE"), Model %in% c("AutoGluon_spatial_var", "AutoGluon"))) +
  geom_tile(aes(x = grouping, y = measure, fill = value), colour = "white") +
  scale_fill_gradient2(low = "indianred3", mid = "white", high = "steelblue") + 
  geom_text(aes(x = grouping, y = measure, label=label)) + 
  theme_minimal() + 
...
ggsave("figures/Reviewer.pdf", width = 7, height = 3, units = c("in"))
ggsave("figures/Reviewer.png", width = 7, height = 3, units = c("in"))
```

```{r}
set.seed(1)

cv_df_a <- filter(cv_df_demean, Model == "AutoGluon_spatial_var")
...
by_day <- day %>% 
  ...
  summarise(R2 = cor(PM, PM_pred)^2, 
            RMSE = mean(sqrt((PM-PM_pred)^2)), 
            NSE = 1 - (mean((PM-PM_pred)^2)/mean((PM-mean(PM))^2)),
            beta = lm(PM ~ PM_pred)$coefficients[2],
            grouping = "Grouped by day") 

print(mean(by_day$R2))
